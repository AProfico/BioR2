...
#   n=10
  ...
  filetype<-strsplit(x = path, split="[.]")[[1]][2]
  
  
  if(filetype=="png"){
  img<-readPNG(path)}else{img<-readJPEG(path)}
...
grid<-round(as.matrix(expand.grid(x,y)),0)
range(x)
plot(grid,cex=3,pch=19,xlim=c(0,10),ylim=c(0,10))
text(grid,labels=1:9,pos=1,cex=2)
sel<-locator(1)
sel<-unlist(sel)
